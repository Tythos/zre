const zre = @import("zre.zig");
const std = @import("std");

const expect = std.testing.expect;

fn encodeStr(comptime encoding: zre.Encoding, comptime str: []const u8) []const encoding.CharT() {
...
fn testMatch(comptime regex: []const u8, comptime encoding: zre.Encoding, comptime str: []const u8) !void {
    const encoded_str = comptime encodeStr(encoding, str);
    try expect((try zre.match(regex, .{ .encoding = encoding }, encoded_str)) != null);
    comptime try expect((try zre.match(regex, .{ .encoding = encoding }, encoded_str)) != null);
}

fn testSearchInner(comptime regex: []const u8, comptime encoding: zre.Encoding, comptime str: []const encoding.CharT(), comptime found: []const encoding.CharT()) !void {
    const result = try zre.search(regex, .{ .encoding = encoding }, str);
    try expect(result != null);
    try expect(std.mem.eql(encoding.CharT(), result.?.slice, found));
}

fn testSearch(comptime regex: []const u8, comptime encoding: zre.Encoding, comptime str: []const u8, comptime found: []const u8) !void {
    const encoded_str = comptime encodeStr(encoding, str);
...
fn testCapturesInner(comptime regex: []const u8, comptime encoding: zre.Encoding, comptime str: []const encoding.CharT(), comptime captures: []const ?[]const encoding.CharT()) !void {
    const result = try zre.match(regex, .{ .encoding = encoding }, str);
    try expect(result != null);
...
fn testCaptures(comptime regex: []const u8, comptime encoding: zre.Encoding, comptime str: []const u8, comptime captures: []const ?[]const u8) !void {
    const encoded_str = comptime encodeStr(encoding, str);
